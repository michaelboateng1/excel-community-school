...
import aduinoImage from "./assets/images/aduinoImage.jpg";
...
const ScoolImages: SchooImages[] = [
  ...
  },
  {
    ...
    day: "TERM TIME",
    image: aduinoImage,
    description: "Blending the best of British, American, and Ghanaian standards to provide a holistic education that prepares students for global opportunities.",
  },
  {
    id: "5",
...
const App: React.FC = () => {
  ...
  return (
    <div className="relative min-h-screen text-white selection:bg-[#4fb7b3] selection:text-black cursor-auto md:cursor-default overflow-x-hidden">
      <CustomCursor />
      <FluidBackground />
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="fixed inset-0 z-30 bg-[#31326f]/95 backdrop-blur-xl flex flex-col items-center justify-center gap-8 md:hidden">
            {["Lineup", "Experience", "Tickets"].map((item) => (
              <button key={item} onClick={() => scrollToSection(item.toLowerCase())} className="text-4xl font-heading font-bold text-white hover:text-[#a8fbd3] transition-colors uppercase bg-transparent border-none">
                {item}
              </button>
            ))}
            <button onClick={() => scrollToSection("tickets")} className="mt-8 border border-white px-10 py-4 text-sm font-bold tracking-widest uppercase bg-white text-black">
              Get Tickets
            </button>

            <div className="absolute bottom-10 flex gap-6">
              <a href="https://x.com/GoogleAIStudio" className="text-white/50 hover:text-white transition-colors">
                Twitter
              </a>
            </div>
          </motion.div>
      ...
      <header className="relative h-[100svh] min-h-[600px] flex flex-col items-center justify-center overflow-hidden px-4">
        <motion.div style={{ y, opacity }} className="z-10 text-center flex flex-col items-center w-full max-w-6xl pb-24 md:pb-20">
          {/* Date / Location */}
          {/* <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 0.2 }} className="flex items-center gap-3 md:gap-6 text-xs md:text-base font-mono text-[#a8fbd3] tracking-[0.2em] md:tracking-[0.3em] uppercase mb-4 bg-black/20 px-4 py-2 rounded-full backdrop-blur-sm">
            <span>Tokyo</span>
          ...
          <div className="relative w-full flex justify-center items-center">
            <GradientText text="Excel Community School" as="h1" className="text-[15vw] md:text-[6vw] leading-[0.9] font-black tracking-tighter text-center" />
        ...
        <div className="max-w-[1600px] mx-auto px-4 md:px-6">
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 md:mb-16 px-4">
            <h2 className="text-5xl md:text-6xl font-heading font-bold uppercase leading-[0.9] drop-shadow-lg break-words w-full md:w-auto">
              Beyound <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#a8fbd3] to-[#4fb7b3]">Excellence</span>
            </h2>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 relative">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 md:gap-16 items-center">
            <div className="lg:col-span-5 order-2 lg:order-1">
              ...
              <p className="text-lg md:text-xl text-gray-200 mb-8 md:mb-12 font-light leading-relaxed drop-shadow-md">Lumina isn't just a festival; it's a sensory expedition. We fuse cutting-edge audio technology with generative art to create a living, breathing ecosystem of sound.</p>
      ...
      <section id="tickets" className="relative z-10 py-20 md:py-32 px-4 md:px-6 bg-black/30 backdrop-blur-lg">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12 md:mb-20">
            <h2 className="text-5xl md:text-9xl font-heading font-bold opacity-20 text-white">ACCESS</h2>
                ...
                <motion.div key={i} whileHover={isDisabled ? {} : { y: -20 }} className={`relative p-8 md:p-10 border border-white/10 backdrop-blur-md flex flex-col min-h-[450px] md:min-h-[550px] transition-colors duration-300 ${ticket.accent} ${isDisabled && !isPurchased ? "opacity-50 grayscale" : ""} will-change-transform`} data-hover={!isDisabled}>
                  ...
                  <div className="flex-1">
                    ...
                    <ul className="space-y-4 md:space-y-6 text-sm text-gray-200">
                      <li className="flex items-center gap-3">
                        <Ticket className="w-5 h-5 text-gray-400" /> General Entry
                      </li>
                      <li className="flex items-center gap-3">
                        <MapPin className="w-5 h-5 text-gray-400" /> All Stages
                      </li>
                      {i > 0 && (
                        <li className="flex items-center gap-3 text-white">
                          <Zap className={`w-5 h-5 text-[#a8fbd3]`} /> Expedited Entry
                        </li>
                      )}
                      {i > 1 && (
                        <li className="flex items-center gap-3 text-white">
                          <Globe className={`w-5 h-5 text-[#4fb7b3]`} /> Backstage Lounge
                        </li>
                      )}
                    </ul>
                  </div>
                  ...
                  <button
                    ...
                    className={`w-full py-4 text-sm font-bold uppercase tracking-[0.2em] border border-white/20 transition-all duration-300 mt-8 group overflow-hidden relative 
                      ${isPurchased ? "bg-[#a8fbd3] text-black border-[#a8fbd3] cursor-default" : isPurchasing ? "bg-white/20 text-white cursor-wait" : isDisabled ? "cursor-not-allowed opacity-50" : "text-white cursor-pointer hover:bg-white hover:text-black"}`}
                  >
      ...
      <AnimatePresence>
        {selectedArtist && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setSelectedArtist(null)} className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-md cursor-auto">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} onClick={(e) => e.stopPropagation()} className="relative w-full max-w-5xl bg-[#1a1b3b] border border-white/10 overflow-hidden flex flex-col md:flex-row shadow-2xl shadow-[#4fb7b3]/10 group/modal">
              {/* Close Button */}
              ...
              <div className="w-full md:w-1/2 p-8 pb-24 md:p-12 flex flex-col justify-center relative">
                <motion.div key={selectedArtist.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.1 }}>
                  ...
                  <p className="text-lg text-[#a8fbd3] font-medium tracking-widest uppercase mb-6">{selectedArtist.category}</p>
